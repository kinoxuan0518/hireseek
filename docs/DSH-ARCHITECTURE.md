# Seeya × DeepSeek Harness

Seeya 没有去 fork 整个 `deepseek-ai/deepseek-harness`（230+ package、developer-preview、接口仍会破）。它走的是 **同一条架构路线**：模型是灵魂，Harness 是让模型理解环境、用工具、持续干活的那一层。一切皆插件。
