...
所以加了一双独立的、对抗性的眼睛（`src/verifier.ts`）：

```bash
hireseek verify            # 换 deepseek-v4-pro 反向质检今日触达
hireseek verify --push     # 顺手把质检结论推给你
```

- **换一个更强、且不同的脑子**：执行用 `v4-flash`，质检用 `v4-pro`——不是同一个判断者
- **不被自评锚定**：验证器只拿岗位画像 + 候选人事实，先独立重判，再回头跟 do-er 的分对比，专抓"自评虚高"
- **代码层再叠 Goodhart 启发式**：凑满目标却低匹配集中、自评显著虚高 → 判定"疑似注水"，主动告警
- 每轮自主寻源（心跳 `run_channel`）跑完会**自动质检**，不通过/有隐患就主动报给你；生命体征里触达数旁边永远跟着一行质检结论——数字好看 ≠ 做得好

这一层不改寻源 agent 任何判断，只在旁边挑刺。它是 Loop Engineering 的"灵魂"在 HireSeek 里的落地：Harness（守护栏，约束"不能怎么做"）+ 独立验证器（盯住"真正的目标"而非可被钻空子的指标）。
