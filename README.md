...
### 🤖 AI 对话与决策
- **对话模式**：直接和 HireClaw 聊，30+ 种工具（触发任务、查候选人、搜背景、改策略）
- **Auto Memory**：跨会话自动记忆，学习你的招聘偏好和成功模式
- **结构化问答**：AI 主动提问明确需求，多选/单选界面友好
- **计划模式**：AI 先分析历史数据、制定策略、用户确认后执行
- **技能系统**：`/找候选人`、`/分析简历`、`/commit` 等快捷命令

### 🔧 开发者工具（Claude Code 级别）
- **Git 自动化**：查看状态、提交代码、创建分支、推送远程、创建 PR
- **搜索工具**：Glob 文件搜索、Grep 内容搜索（优先 ripgrep）
- **MCP 生态集成**：支持 MCP 协议，连接文件系统、GitHub、Slack 等各种服务
- **PDF 阅读**：直接分析候选人简历 PDF
- **代码自省**：能读写自己的源代码（`src/`），TypeScript 校验，写错自动回滚
- **任务管理**：结构化任务、层级关系、可视化看板

### 🛡️ 稳定性与智能
- **错误自动恢复**：检测验证码/登录过期，智能重试，断点续传
- **实时控制台**：Web dashboard 看实时截图、LLM 推理日志，跑着跑着能发介入指令
...
- **[完整功能文档](./docs/COMPLETE-FEATURES.md)** - HireClaw vs Claude Code 100% 对比
- **[Claude Code 功能](./docs/CLAUDE-CODE-FEATURES.md)** - 详细功能说明
- **[Git 自动化指南](./docs/GIT-AUTOMATION.md)** - Git 操作使用指南
- **[MCP 协议指南](./docs/MCP-GUIDE.md)** - MCP 集成使用指南
- **[实现总结](./docs/IMPLEMENTATION-SUMMARY.md)** - 技术实现总结
