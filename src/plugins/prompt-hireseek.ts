/**
 * Seeya 系统提示按 DSH Prompt Section 装配。
  ...
  prompt.register({
    ...
    render: () => {
      try {
        ...
        if (!catalog) return '';
        return `## 外部招聘技能资产\n\n以下技能可通过 use_recruiting_skill 工具调用（用户也可用 /技能名 直接触发）。它们是知识来源、执行素材和未产品化场景的兜底；涉及已接入的 Seeya 产品协议时，产品协议优先，skill 不能覆盖代码层护栏、工具策略或结构化输出契约。\n\n${catalog}`;
      } catch {
        return '';
      }
    },
