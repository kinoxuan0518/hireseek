...
describe('dsh agent loop', () => {
  ...
  it('projects reasoning_content back into assistant history after a turn', async () => {
    const ctx = new Context();
    ctx.plugin({ name: 'sessions', apply: sessionPlugin });
    ctx.plugin({ name: 'tools', apply: toolsPlugin });
    ctx.plugin({ name: 'llm', apply: llmPlugin });
    ...
    ctx.get<LLMService>('llm').register({
      id: 'mock',
      async complete(): Promise<LLMResponse> {
        return { content: 'ok', reasoning_content: 'hidden-thought' };
      },
    });
    ctx.get<LLMService>('llm').setDefaultModel('mock');
    const session = ctx.get<SessionService>('sessions').create({ source: 'test' });
    await ctx.get<AgentLoop>('agentLoop').runTurn({
      session,
      input: 'hi',
      preserveSystem: true,
    });
    const assistant = session.deriveMessages().find(m => m.role === 'assistant') as { reasoning_content?: string };
    expect(assistant.reasoning_content).toBe('hidden-thought');
  });
...
describe('seeya dsh runtime', () => {
  ...
  it('does not mount seeya prompt sections on the subagent profile', () => {
...
describe('session hydrate', () => {
  it('round-trips existing OpenAI messages into the event log', () => {
    ...
    expect((messages[2] as { reasoning_content?: string }).reasoning_content).toBe('think-step');
  });
});
